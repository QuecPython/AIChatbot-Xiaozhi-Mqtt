...
class MqttClient(object):
    def __init__(self):
        ...
        self.ip = dataCall.getInfo(1,0)[2][2]
    ...
    def ota_get(self):
        ...
        self._sub_topic = 'devices/p2p/'+ self.get_mac_address()
    ...
    def mcp_send(self,payload):
        data = JsonMessage({
            #'session_id': aes_opus_info['session_id'],
            'type': 'mcp',
            'payload': payload
        }).to_bytes()
        mcp_topic = 'devices/p2p/'+ self.get_mac_address()
        self.cli.publish(mcp_topic,data)

    def mcp_initialize(self,id):
        payload = {
            "jsonrpc": "2.0",
            "id": id,
            "result": {
        ...
        self.mcp_send(payload)

    def mcp_tool_list(self, id):
        payload = {
            "jsonrpc": "2.0",
            "id": id,
            "result": {
                ...
                "tools": [
                {
                    ...
                    "description": "Get the volume of the audio speaker.",
                    "parameters": {}
                },
                {
                    "name": "self.audio_speaker.set_volume",
                    "description": "Set the volume of the audio speaker,Volume range is from 0 to 11.",
                    "parameters": {
                    "volume": {
                        "type": "integer",
                        "minimum": 0,
                ...
                {
                    "name": "self.dev.get_device_info",
                    "description": "Get the information of the device.",
                    "parameters": {}
                },
                {
                    "name": "self.dev.get_device_ip",
                    "description": "Get the IP address of the device.When it is necessary to obtain the device's location, one should first call this tool to obtain the device's IP address, and then obtain the device's location.",
                    "parameters": {}
                }
                ],
            }
    ...
    def mcp_tool_call(self, id, method_name='', iserror = False,result={}):
        ...
        if not iserror:
            if tool_name == "self.audio_speaker.get_volume":
                payload = {
                    ...
                    "result": {
                        "content": [
                            { "type": "text", "text": "当前音量为 {}".format(result['volume']) }
                        ],
                        "isError": False
                    }
                }
            elif tool_name == "self.audio_speaker.set_volume":
                self.current_volume = result['volume']
                payload = {
                    "jsonrpc": "2.0",
                    "id": id,
                    "method": "tools/call",
                    "result": {
                        "content": [
                            { "type": "text", "text": "当前音量已设置为 {}".format(result['volume']) }
                        ],
                        "isError": False
                    }
                }
            elif tool_name == "self.dev.get_device_info":
                payload = {
                    "jsonrpc": "2.0",
                    "id": id,
                    "method": "tools/call",
                    "result": {
                        "content": [
                            { "type": "text", "text": "名称:移远通信EC800M系列AI开发板,状态:正在运行小智AI,当前音量为 {}".format(self.current_volume)}
                        ],
                        "isError": False
                    }
                }
            elif tool_name == "self.dev.get_device_ip":
                payload = {
                    "jsonrpc": "2.0",
                    "id": id,
                    "method": "tools/call",
                    "result": {
                        "content": [
                            { "type": "text", "text": "设备IP地址为 {}".format(self.ip) }
                        ],
                        "isError": False
                    }
                }
    ...
    def handle_mcptool_message(self, method, id):
        
        if method == 'initialize':
            self.mcp_initialize(id)
        elif method == 'tools/list':
            self.mcp_tool_list(id)
            #logger.debug("发送MCP tools/list响应消息")
        elif method == 'notifications/initialized':
            logger.info('MCP initialized notification received')
        elif method == "notifications/cancelled":
            logger.info('MCP cancelled notification received')
